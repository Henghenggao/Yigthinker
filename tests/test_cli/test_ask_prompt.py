...
from yigthinker.cli.ask_prompt import ask_user_permission, PermissionAnswer


async def test_user_approves():
    with patch("yigthinker.cli.ask_prompt._prompt_user", return_value="y"):
...
async def test_user_denies():
    with patch("yigthinker.cli.ask_prompt._prompt_user", return_value="n"):
...
async def test_user_allows_all():
    with patch("yigthinker.cli.ask_prompt._prompt_user", return_value="a"):
