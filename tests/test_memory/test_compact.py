...
async def test_no_compact_below_threshold():
    ...
    result = await compact.run(messages, memory_content="", token_estimate=1000)
...
async def test_compacts_when_over_threshold():
    ...
    result = await compact.run(messages, memory_content="## Project Memory\nSome facts.", token_estimate=50000)
    assert len(result) < len(messages)
    assert "[Session Memory" in result[0].content  # memory injection happened


async def test_memory_injected_as_first_message():
    compact = SmartCompact(CompactConfig(max_tokens=100, min_tokens=0, min_text_block_messages=0))
    messages = _make_messages(10)
    result = await compact.run(messages, memory_content="Key fact: revenue is 1M.", token_estimate=50000)
    assert "[Session Memory" in result[0].content


async def test_vars_summary_appended_to_memory_message():
    compact = SmartCompact(CompactConfig(max_tokens=100, min_tokens=0, min_text_block_messages=0))
    messages = _make_messages(10)
    result = await compact.run(
        messages,
...
async def test_fallback_truncation_when_memory_empty():
    compact = SmartCompact(CompactConfig(max_tokens=100, min_tokens=0, min_text_block_messages=0))
    messages = _make_messages(10)
    result = await compact.run(messages, memory_content="", token_estimate=50000)
