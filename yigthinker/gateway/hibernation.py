...
def _save_var(name: str, value: Any, vars_dir: Path) -> dict[str, Any]:
    ...
    if isinstance(value, pd.DataFrame):
        ...
        try:
            ...
            # Fallback for mixed-type columns or other pyarrow issues
            logger.warning("Parquet failed for '%s', falling back to pickle", name)
            path = vars_dir / f"{name}.pickle"
            with open(path, "wb") as f:
                pickle.dump(value, f, protocol=5)
            return {"format": "pickle", "file": path.name, "shape": list(value.shape)}
    ...
    with open(path, "wb") as f:
        pickle.dump(value, f, protocol=5)
    logger.warning("Used pickle fallback for variable '%s' (type=%s)", name, type(value).__name__)
    return {"format": "pickle", "file": path.name}


def _load_var(name: str, entry: dict[str, Any], vars_dir: Path) -> Any:
    """Load a single variable from its manifest entry."""
    fmt = entry.get("format", "")
    path = vars_dir / entry["file"]
